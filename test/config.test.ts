...
test('production config rejects unsafe public defaults', () => {
  assert.throws(
    ...
    /ARIADNE_STORAGE=firestore.*CORS_ORIGINS=\*.*ARIADNE_ALLOW_MOCK_PROVIDER=true.*ARIADNE_PAID_USAGE_ENABLED=true.*ARIADNE_FIREBASE_AUTH_REQUIRED=true.*GEMINI_API_KEYS.*APP_URL.*STRIPE_SECRET_KEY.*STRIPE_WEBHOOK_SECRET/
  );
});
