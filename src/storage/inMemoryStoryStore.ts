...
export class InMemoryStoryStore implements StoryStore {
  ...
  async listAudioUploadIntents(repoId: string, branchId?: string): Promise<AudioUploadIntent[]> {
    if (!this.repos.has(repoId)) throw new StoreError(`repo not found: ${repoId}`, 'not_found');
    return [...this.audioUploads.values()]
      .filter(intent => intent.repoId === repoId && (branchId === undefined || intent.branchId === branchId))
      .sort((a, b) => a.createdAt.localeCompare(b.createdAt))
  ...
  async linkAudioAssetToTurn(input: {
    ...
    const asset = this.audioAssets.get(input.audioAssetId);
